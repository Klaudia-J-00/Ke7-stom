  ...
  const submitHandler = (e) => {
    ...
    if (password !== confirmPassword) {
      if (!toast.isActive(toastId.current)) {
        //prevents from creating multiple toasts
        toastId.current = toast.error("Hasła nie są takie same", Toastobjects);
      }
    } else {
      dispatch(
        updateUserProfile({ id: user._id, name, surname, email, password })
      );
      if (!toast.isActive(toastId.current)) {
        //prevents from creating multiple toasts
            ...
            <div className="col-md-12 mb-5">
              ...
              {loading ? (
                ...
                <>
                  {orders.length === 0 ? (
                    <h5>
                      Brak zamówień <Link to="/keyboard">zrób zakupy!</Link>
                    </h5>
                  ) : (
                    <>
                        ...
                        <table className="table">
                          ...
                          <tbody>
                            {orders.map((order) => (
                              <tr
                                className={`${
                                  order.isPaid ? "green-order" : "red-order"
                                }`}
                                key={order._id}
                              >
                                <td>
